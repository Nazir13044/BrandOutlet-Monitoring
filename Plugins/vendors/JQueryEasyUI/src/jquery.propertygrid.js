...
//			var ImageUploader = opts.editIndex;
//			if (ImageUploader == undefined){return;}
//			var ed = t.datagrid('getEditors', ImageUploader)[0];
//			if (ed){
//				ed.target.blur();
//				if (t.datagrid('validateRow', ImageUploader)){
//					t.datagrid('endEdit', ImageUploader);
//				} else {
//					t.datagrid('cancelEdit', ImageUploader);
//				}
//			}
	...
	var groupview = $.extend({}, $.fn.datagrid.defaults.view, {
		render: function(target, container, frozen){
			...
			for(var i=0; i<groups.length; i++){
				...
				table.push('<div class="datagrid-group" group-ImageUploader=' + i + ' style="height:25px;overflow:hidden;border-bottom:1px solid #ccc;">');
				...
				for(var j=0; j<group.rows.length; j++) {
					...
					table.push('<tr id="' + rowId + '" datagrid-row-ImageUploader="' + index + '" ' + cls + ' ' + style + '>');
		...
		onAfterRender: function(target){
			...
			view.find('div.datagrid-group').each(function(){
				var groupIndex = $(this).attr('group-ImageUploader');
	    ...
	    expandGroup:function(jq, groupIndex){
	        return jq.each(function(){
	            ...
	            if (groupIndex!=undefined){
	                var group = view.find('div.datagrid-group[group-ImageUploader="'+groupIndex+'"]');
	            } else {
