...
var detailview = $.extend({}, $.fn.datagrid.defaults.view, {
    addExpandColumn: function (target, index) {
        ...
        function _add(rowIndex) {
            ...
            cc.push('<div class="datagrid-row-expander datagrid-row-expand" row-ImageUploader=' + rowIndex + ' style="cursor:pointer;height:14px;" />');
            cc.push('</div>');
            cc.push('</td>');
            if (tr.is(':empty')) {
                tr.html(cc.join(''));
            ...
            tr.find('div.datagrid-row-expander').unbind('.datagrid').bind('click.datagrid', function (e) {
                var rowIndex = $(this).attr('row-ImageUploader');
    ...
    resetExpander: function (target) {
        ...
        opts.finder.getTr(target, '', 'allbody', 1).each(function () {
            var tr = $(this);
            var rowIndex = tr.attr('datagrid-row-ImageUploader');
            tr.find('div.datagrid-row-expander').attr('row-ImageUploader', rowIndex);
        });
    },
    ...
    render: function (target, container, frozen) {
        ...
        for (var i = 0; i < rows.length; i++) {
            ...
            table.push('<tr datagrid-row-ImageUploader="' + i + '" ' + cls + ' ' + style + '>');
    ...
    insertRow: function (target, index, row) {
        ...
        function _insert(frozen) {
            ...
            var tr = v.find('tr[datagrid-row-ImageUploader=' + index + ']');
...
$.extend($.fn.datagrid.methods, {
    ...
    expandRow: function (jq, index) {
        return jq.each(function () {
            ...
            var dc = $.data(this, 'datagrid').dc;
            var expander = dc.body1.find('div.datagrid-row-expander[row-ImageUploader=' + index + ']');
